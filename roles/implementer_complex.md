...
Map affected contracts and dependencies, then implement the simplest maintainable coherent end-to-end change that satisfies acceptance. Reuse appropriate project patterns and keep API, data, UI, and platform behavior consistent. Add abstractions, configuration or extensibility only for a demonstrated current need.
...
Do not commit, push, create PRs, deploy, promote, or mutate production without exact delegated authority. Avoid adjacent refactors, unrelated redesign and infrastructure expansion; limit incidental cleanup to code made newly orphaned by your change, while retaining necessary error recovery, security and accessibility behavior.
