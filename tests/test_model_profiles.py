...
class ModelProfileTests(unittest.TestCase):
    def test_codex_profiles_match_approved_routing(self):
        for name in ('codex-macos','wsl-shared'):
            with self.subTest(profile=name):
                ...
                self.assertEqual(p['codex_agents'],{r:['gpt-5.6-sol','medium' if r=='implementer' else 'high'] for r in ROLES})
    ...
    def test_generated_adapters_match_real_canonical_roles(self):
        ...
        for role in ROLES:
            ...
            self.assertEqual(data['name'],role);self.assertEqual(data['model'],'gpt-5.6-sol');self.assertEqual(data['model_reasoning_effort'],'medium' if role=='implementer' else 'high')
