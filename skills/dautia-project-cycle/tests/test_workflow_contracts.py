...
class WorkflowContractTests(unittest.TestCase):
    ...
    def test_recent_friction_correctives_are_contractual(self) -> None:
        ...
        self.assertIn("no obliga a consultar changelogs", agents)
        self.assertIn("origin/<integration_branch>", agents)
        self.assertIn("Dependency-only upgrades default", dautia)
        self.assertIn("extra browsers/viewports", dautia)
        self.assertIn("source revision, target, provider", release)
        self.assertIn("nunca crea otro `release_operator`", agents)
        self.assertIn("stale_contract", agents)
        self.assertIn("delegar por oleadas", agents)
        self.assertIn("pruebas verdes no prueban aceptacion visual", agents)
        self.assertIn("muestra representativa o contact sheet", agents)
        self.assertIn("must never delegate to another release operator", dautia)
        self.assertIn("delegate in waves", dautia)
        self.assertIn("green technical tests do not prove visual acceptance", dautia)
        self.assertIn("Never spawn agents", release)
        self.assertIn("10.000 caracteres", agents)
