...
## Corpus y calibración

Las replays de Jev pueden guardarse como un objeto con `cases`. Cuando exista una
etiqueta independiente, `gold_profile` registra el perfil que el principal y el
revisor consideran adecuado; no se deriva de la respuesta de Jev. Los casos sin
etiqueta o sin recomendación permanecen desconocidos. `scripts/evidence_metrics.py`
resume estados, cobertura y precisión solo sobre pares comparables, además de
latencia, fallbacks y acciones no autorizadas:

```sh
python3 skills/dautia-project-cycle/scripts/evidence_metrics.py replay.json
```

El resumen no ajusta umbrales, activa `selective` ni convierte una abstención en
un fallo. Sirve para comparar shadow/selective después de reunir suficientes casos
reales con evidencia y revisión independiente.
